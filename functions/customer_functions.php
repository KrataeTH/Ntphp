...
function saveCustomer($pdo, $data) {
    if (!empty($data['customer_id'])) {
        ...
        $stmt->execute([
            ...
            ':create_at' => $data['create_at'], // ส่งค่า create_at
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES 
            (:customer_name, :customer_type, :customer_phone, :customer_status, :address_id, :create_at, CURRENT_DATE)
        ");
        $stmt->execute([
            ':customer_name' => $data['customer_name'],
            ':customer_type' => $data['customer_type'],
            ...
            ':customer_status' => $data['customer_status'],
            ':address_id' => $data['address_id'],
            ':create_at' => $data['create_at'] // ใช้ค่าที่ส่งจากฟอร์ม
        ]);
    }
}
...
function getAmphures($pdo) {
    $stmt = $pdo->prepare("SELECT amphure_id, amphure_name FROM amphure ORDER BY amphure_name ASC");
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
